...
static TEE_Result fp_call_syscall(void *arg __unused)
{
	TEE_Time t = { };
...
static TEE_Result fp_call_rpc(void *arg)
{
	uint32_t *ms = arg;
...
static TEE_Result fp_call_crypto(void *arg __unused)
{
				     ...
				     TEE_Result (*fn)(void *), void *arg)
{
	...
	if (res != TEE_SUCCESS) {
		EMSG("FP context: call failed: %#" PRIx32, res);
		return res;
	}
